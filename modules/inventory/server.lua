...
function Inventory.SetSlot(inv, item, count, metadata, slot)
	...
	if currentSlot and newCount < 1 then
		...
		currentSlot = {name = item.name, label = item.label, rarity = item.rarity, weight = item.weight, slot = slot, count = newCount, description = item.description, metadata = metadata, stack = item.stack, close = item.close}
...
local function generateItems(inv, invType, items)
	...
	for i = 1, #items do
		...
		if not item then
			...
			returnData[i] = {name = item.name, label = item.label, rarity = item.rarity, weight = weight, slot = i, count = count, description = item.description, metadata = metadata, stack = item.stack, close = item.close}
		end
	end
	...
	if result then
		...
		for _, v in pairs(result) do
			...
			if item then
				...
				weight += slotWeight
				returnData[v.slot] = {name = item.name, label = item.label, rarity = item.rarity, weight = slotWeight, slot = v.slot, count = v.count, description = item.description, metadata = v.metadata, stack = item.stack, close = item.close}
			end
		end
	end
...
function Inventory.SetItem(inv, item, count, metadata)
	...
	if item and count >= 0 then
		...
		if inv.player and (inv.weight + (item.weight*count)) > shared.playerweight then
            lib.notify(inv.player.source, {
                type = 'error', 
                description = 'Your inventory does not have enought space!!'
            })
        	return
    	end
