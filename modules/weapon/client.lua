...
local function requiresVehicleCheck(weapon)
    for _, weap in pairs(vehicleCheckWeapons) do
        if weapon == GetHashKey(weap) then
            return true
        end
    end
    return false
...
function Weapon.Equip(item, data)
    ...
    if requiresVehicleCheck(data.hash) then
		local vehicle = VehicleInFront()
		if vehicle == -1 then
			lib.notify({
				id = 'trunkweapon',
				title = 'Weapon',
				description = 'You must be near a vehicle trunk to draw this weapon.',
				showDuration = true,
				position = 'bottom',
				icon = 'gun',
			})
	...
	if client.weaponanims then
        ...
        if anim == anims[`GROUP_PISTOL`] or data.type == "side" then
            if GetConvar('malisling:enable_sling', 'false') == 'true' then
                ...
                lib.showTextUI('[RMOUSE] - Un [BACKSPACE] - Ca', {icon = 'hand'})
