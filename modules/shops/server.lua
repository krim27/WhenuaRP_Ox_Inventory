		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				if string.find(fromData.name, "WEAPON_") then
					local creator = shop.label
					local name = fromItem.label
					local serial = metadata.serial
					--local imageurl = ("https://cfx-nui-ox_inventory/web/images/%s.png"):format(fromData.name)
					local notes = "Purchased from " .. shop.label
					local owners = { {identifier = playerInv.owner, name = firstname.. " " .. lastname} }
					AddWeaponToMDT(creator, name, owners, serial, notes)
				end
...
function AddWeaponToMDT(creator, name, owners, serial, notes)
    Citizen.CreateThread(function()
        Wait(500)  -- Optional delay if needed
        ...
        local success, result = pcall(function()
            ...
            local weaponData = {
                creator = creator,
                name = name,
                owners = owners,  -- Convert owners table to JSON string
                serial = serial,
                notes = notes
            }

            -- Register the weapon with MDT
            exports.tk_mdt:registerWeapon(weaponData)
        end)

        if not success then
            print("Unable to add weapon to MDT: " .. tostring(result))
        end
    end)
end
