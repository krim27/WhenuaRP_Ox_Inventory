		...
		for _, v in pairs(data) do
			if type(v) == 'number' or not v.count or not v.slot then
				...
				if item then
					...
					inventory[v.slot] = {name = item.name, label = item.label, rarity = item.rarity, weight = weight, slot = v.slot, count = v.count, description = item.description, metadata = v.metadata, stack = item.stack, close = item.close}
				end
			end
		end
	end
		...
		if durability and consume then
			if durability > 100 then
				...
				if ostime > durability then
					...
					return TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = locale('no_durability', label), position = 'bottom' })
				elseif consume ~= 0 and consume < 1 then
					...
					if percentage < consume * 100 then
						return TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = locale('not_enough_durability', label), position = 'bottom' })
					end
				end
			...
			if data.count > 1 and consume < 1 and consume > 0 and not Inventory.GetEmptySlot(inventory) then
				return TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = locale('cannot_use', label), position = 'bottom' })
			end
		end

		if item and data and data.count > 0 and data.name == item.name then
			data = {name=data.name, label=label, rarity = rarity, count=data.count, slot=slot, metadata=data.metadata, weight=data.weight}
			...
			elseif consume then
				if data.count >= consume then
					...
					return TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = locale('item_not_enough', item.name), position = 'bottom' })
				end
